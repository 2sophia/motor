...
| 9  | [interrupt](./interrupt)                     | Cancel an in-flight run — `motor.interrupt()` + `was_interrupted` flag.  |
| 10 | [concurrency](./concurrency)                 | Fan out N independent runs across N motors with `asyncio.gather`.        |
